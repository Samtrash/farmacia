...
document.addEventListener("DOMContentLoaded", function(){
    ...
    function cargarDatos(){
        ...
        fetch(`<?= base_url('cliente/listar') ?>?filtro=${f}&buscar=${b}`)
        .then(res => res.json())
        .then(data => {
            ...
            data.forEach(dato => {
                ...
                <div class="row">
                    ...
                    <div class="fila">
                        <div>${c++}</div>
                        <div>${dato.dni_rut ? dato.dni_rut : ''}</div>
                        <div>${dato.nombre_razon}</div>
                        <div>${dato.celular1 ? dato.celular1 : ''}</div>
                        <div>${dato.celular2 ? dato.celular2 : ''}</div>
...
let wzSteps = [
{
    id: "cliente",
    render: (data = {}) => `
    <h1>${modo === 'crear' ? 'Nuevo cliente' : 'Editar cliente'}</h1>
    ...
    <div class="field">
        <em>Celular1</em>
        <input type="text" id="celular1" name="celular1" inputmode="numeric" pattern="[0-9]*" placeholder="999 999 999" maxlength="11" oninput="trionumero(event)" value="">
    </div>

    <div class="field">
        <em>Celular2</em>
        <input type="text" id="celular2" name="celular2" inputmode="numeric" pattern="[0-9]*" placeholder="999 999 999" maxlength="11" oninput="trionumero(event)" value="">
    </div>
    ...
    validate: () => {
        ...
        let celular1 = document.getElementById('celular1').value.trim();
        let celular2 = document.getElementById('celular2').value.trim();
...
function guardarCliente(data){
    ...
    let url = modo === 'crear'
        ? "<?= base_url('cliente/create') ?>"
        : "<?= base_url('cliente/update') ?>/" + idActual;
    ...
    fetch(url, {
        ...
        body: "dni_rut="+encodeURIComponent(data.dni_rut)+
              "&nombre_razon=" + encodeURIComponent(limpiarTexto(data.nombre_razon))+
              "&celular1="+encodeURIComponent(data.celular1)+
              "&celular2="+encodeURIComponent(data.celular2)
    })
    .then(res => res.json())
    .then(res => {
...
function eliminarcliente(id){
    ...
    fetch("<?= base_url('cliente/delete') ?>/" + id, {
        method: 'POST'
    })
    .then(res => res.json())
    .then(res => {
